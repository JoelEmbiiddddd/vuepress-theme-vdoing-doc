---
title: 手写RPC框架
date: 2023-07-11 10:45:02
permalink: /pages/8d4c04/
---
# 手撸RPC

## 基本结构

![image-20230711105951895](./image/image-20230711105951895.png)
...
![image-20230711110015497](./image/image-20230711110015497.png)
...
所以，网络传输会出现这样：  
![image-20230711110029307](./image/image-20230711110029307.png)1.服务端恰巧读到了两个完整的数据包 A 和 B，没有出现拆包/粘包问题；  
...
- 消息长度 + 消息内容  
  ...
  ![image-20230711110042229](./image/image-20230711110042229.png)
...
- 空间上  
  ![image-20230711110101813](./image/image-20230711110101813.png)

- 时间上  
  ![image-20230711110113853](./image/image-20230711110113853.png)
...
- Sync 同步调用。
  ...
  ![image-20230711110123047](./image/image-20230711110123047.png)

- Future 异步调用    
  客户端发起调用后不会再阻塞等待，而是拿到 RPC 框架返回的 Future 对象，调用结果会被服务端缓存，客户端自行决定后续何时获取返回结果。当客户端主动获取结果时，该过程是阻塞等待的
  ![image-20230711110129131](./image/image-20230711110129131.png)

- Callback 回调调用
  客户端发起调用时，将 Callback 对象传递给 RPC 框架，无须同步等待返回结果，直接返回。当获取到服务端响应结果或者超时异常后，再执行用户注册的 Callback 回调 
  ![image-20230711110134096](./image/image-20230711110134096.png)

- Oneway 单向调用
  客户端发起请求之后直接返回，忽略返回结果  
  ![image-20230711110138849](./image/image-20230711110138849.png)      
